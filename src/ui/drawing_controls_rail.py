...
def DrawingControls(settings, story) -> list[ft.control]:
    ...
    def save_custom_brush_clicked(e=None):
        ...
        content = ft.Column([new_custom_brush_name_text_field], scroll=ft.ScrollMode.AUTO, height=page.height / 2) 

        dlg = ft.AlertDialog(
            title=ft.Text("Name your custom brush"), 
            content=content,
            actions=[
                ft.TextButton("Cancel", on_click=lambda _: page.pop_dialog(), style=ft.ButtonStyle(color=ft.Colors.ERROR, mouse_cursor="click")),
                save_button
            ]
        )
    ...
    def get_text_options() -> list[ft.Control]:
        ...
        def save_custom_text_settings_clicked(e=None):
            ...
            content = ft.Column([new_custom_text_setting_name_text_field], scroll=ft.ScrollMode.AUTO, height=page.height / 2)
