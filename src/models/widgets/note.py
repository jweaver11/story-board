...
class Note(Widget):
    ...
    def build(self):
        ...
        async def create_segment(e=None):
            self.data['segment_data'].append({"title": self.new_segment_tf.value, "content": ""})
        ...
        async def delete_segment(e: ft.Event):
            ...
            if len(self.data['segment_data']) > index:
                del self.data['segment_data'][index]
                self.update_data(**{'segment_data': self.data['segment_data']})
        ...
        async def save_segment(e):
            index = e.control.data
            if len(self.data['segment_data']) > index:
                self.data['segment_data'][index]['content'] = e.control.value
                self.update_data(**{'segment_data': self.data['segment_data']})
