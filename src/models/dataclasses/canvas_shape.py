...
class CanvasShape(ft.Container):
    ...
    async def _manipulate(self, e: ft.DragUpdateEvent):
        ...
        match self.manipulate_action:
            ...
            case "resize_right":  # left edge stays fixed
                angle = self.rotate.angle
                cos_a, sin_a = math.cos(angle), math.sin(angle)
                dx = e.local_delta.x
                self.canvas.width = max(20, self.canvas.width + dx)
    ...
    def build(self):
        ...
        self.canvas = cv.Canvas(
            shapes=[],      # Shape we built depending on the tool being used
            content=ft.GestureDetector(
                ft.Container(expand=True, border=ft.Border.all(2, ft.Colors.BLACK)),
